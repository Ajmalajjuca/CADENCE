...
function AboutStep({ form, update }: { form: ProfileFormState; update: Update }) {
  return <>
    <header className="page-heading"><h1 className="display-heading">What should we call you?</h1><p className="page-intro">A little context helps Cadence write from your point of view.</p></header>
...
function AudienceStep({ form, update }: { form: ProfileFormState; update: Update }) {
  return <>
    <header className="page-heading"><h1 className="display-heading">Who do you want your posts to reach?</h1><p className="page-intro">Choose a starting point or describe the exact people you want to help.</p></header>
...
function TopicsStep({ form, update }: { form: ProfileFormState; update: Update }) {
  ...
  return <>
    <header className="page-heading"><h1 className="display-heading">What topics do you want to be known for?</h1><p className="page-intro">Add up to four. The first topic is your primary theme.</p></header>
    <ol className="space-y-3" aria-label="Content topics">
      {form.pillars.map((pillar, index) => <li key={`${pillar}-${index}`} className="flex flex-wrap items-center gap-2 rounded-[0.8rem] border border-[var(--border)] bg-[var(--surface-muted)] p-3">
...
function VoiceStep({ form, update }: { form: ProfileFormState; update: Update }) {
  ...
  return <>
    <header className="page-heading"><h1 className="display-heading">What does your natural writing sound like?</h1><p className="page-intro">Paste something you wrote yourself, then choose a few traits. You can skip this and add samples later.</p></header>
...
function PreferencesStep({ form, update }: { form: ProfileFormState; update: Update }) {
  return <>
    <header className="page-heading"><h1 className="display-heading">How should Cadence shape each post?</h1><p className="page-intro">These are useful defaults, not rigid rules. Every choice can be changed later.</p></header>
...
function StoriesStep({ form, update }: { form: ProfileFormState; update: Update }) {
  ...
  return <>
    <header className="page-heading"><h1 className="display-heading">Which real experiences may Cadence draw from?</h1><p className="page-intro">Cadence may use only the details you add here. It will never invent personal experiences.</p></header>
    <div className="space-y-4">{form.stories.map((story, index) => <section key={index} className="rounded-[var(--radius-card)] border border-[var(--border)] bg-[var(--surface-muted)] p-5" aria-labelledby={`story-${index + 1}-heading`}>
