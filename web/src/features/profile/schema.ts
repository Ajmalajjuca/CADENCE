...
export const onboardingInput = z.discriminatedUnion("step", [
  ...
  z.object({ step: z.literal(4), samples: z.array(nonblank).max(5), voiceTraits: z.array(z.string().max(100)).max(12) }),
  z.object({ step: z.literal(5), rules: z.object({ lengthPreference: text, casing: text, hashtags: text, emoji: text, cta: text, bannedTerms: z.array(z.string().max(100)).max(30), notes: text }) }),
